...
import {
  ...
  TRADER_AI_ENTRY_ERROR_CODE_MESSAGES,
  TraderAIEntryConfig,
  TraderAIEntryResult,
  loadTemplates,
  saveTemplate,
 ...
 */
async function executeApi(config: TraderAIEntryConfig): Promise<GenerationResult> {
  console.log('[TraderAIEntry] Execute API with config:', config);

  const result = await executeTraderAIEntry(config);
...
export const TraderAIEntryPage: React.FC<TraderAIEntryPageProps> = ({
  ...
  const workflowConfig = useMemo((): GenerateWorkflowConfig<TraderAIEntryConfig, TraderAIEntryState> => ({
    ...
    executeApi,
    buildStorageRequest,
    errorMessages: TRADER_AI_ENTRY_ERROR_CODE_MESSAGES,
