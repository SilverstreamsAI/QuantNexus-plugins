  ...
  return (
    <section className="p-6 rounded-lg border border-color-terminal-border bg-color-terminal-surface/30">
      ...
      <div className="min-h-[80px] p-4 rounded-lg border border-dashed border-color-terminal-border bg-color-terminal-surface/20 mb-4">
        <div className="grid grid-cols-5 gap-3">
          ...
          >
            ...
            <span className="text-xs">{t('factorFactory.addFactor')}</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 p-3 rounded-lg bg-color-terminal-surface/10 border border-color-terminal-border/50">
        <label className="text-sm text-color-terminal-text-secondary">{t('factorFactory.combinator')}</label>
        <select
          value={method}
        ...
        >
          {FACTOR_COMBINATOR_METHODS.map(m => (
            <option key={m.id} value={m.id}>{t(`combinatorMethods.factor.${m.id}`)}</option>
          ))}
        </select>
        <label className="text-sm text-color-terminal-text-secondary ml-4">{t('factorFactory.lookback')}</label>
        <input
          type="number"
          value={lookback}
        ...
        />
        <span className="text-sm text-color-terminal-text-secondary">{t('factorFactory.days')}</span>
      </div>
    </section>
  );
};
